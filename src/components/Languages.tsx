...
export default function Languages() {
  ...
  return (
    <div className='w-full bg-black text-zinc-600 pt-[30px] pb-[150px] flex flex-col items-center overflow-hidden relative'>
            ...
            <motion.div key={index} className="flex-shrink-0 w-full">
              <ul className="flex justify-center space-x-[30px] screen12:space-x-[50px] spacesliders:space-x-0 spacesliders:justify-around">
                {slide.map((language: Language) => (
                  <li key={language.id} className='flex items-center space-x-2 spacesliders:space-x-4'>
                    <Image
                      src={language.image}
                      alt={language.name}
                      width={50}
